...
const AddProduct = () => {
    ...
    if (!nameInput.value.trim()) {
        ...
        document.getElementById('error-name').textContent = 'Item Name is required.'; // [1 mark]
        isValid = false;
    }
    
    // Check for empty or non positive quantity
    if (!quantityInput.value || parseFloat(quantityInput.value) <= 0) { 
        document.getElementById('error-quantity').textContent = 'Quantity must be a positive number.'; // [1 mark]
        isValid = false;
    }
    
    // Check for empty or non positive price
    if (!priceInput.value || parseFloat(priceInput.value) <= 0) {
        document.getElementById('error-price').textContent = 'Price Per Unit must be a positive number.'; // [1 mark]
        isValid = false;
    }
    ...
    // Reset grand total display to $0.00, as total hasn't been re-calculated yet (Sample UI 49)
    document.getElementById('grand total display').textContent = 'Total: $0.00';
};
